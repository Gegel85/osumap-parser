...
char	**OsuMap_splitString(char *str, char separator, char *error_buffer, jmp_buf jump_buffer)
{
	...
	if (!array){
		sprintf(error_buffer, "Memory allocation error (%luB)", 2 * sizeof(*array));
...
OsuMapCategory	*OsuMap_getCategories(char **lines, char *error_buffer, jmp_buf jump_buffer)
{
	...
	if (!lines[0]) {
		...
		if (!categories) {
			sprintf(error_buffer, "Memory allocation error (%luB)", sizeof(*categories));
			longjmp(jump_buffer, true);
		}
	}
	for (int i = 0; lines[i]; i++)
	...
	if (!categories) {
		sprintf(error_buffer, "Memory allocation error (%luB)", (len + 1) * sizeof(*categories));
		longjmp(jump_buffer, true);
	}
	for (size_t i = 0; lines[i]; currentIndex++) {
		...
		if (!categories[currentIndex].lines) {
			sprintf(error_buffer, "Memory allocation error (%luB)", (i - start + 1) * sizeof(*categories[currentIndex].lines));
			longjmp(jump_buffer, true);
		}
...
OsuMap_unsignedIntegerArray	OsuMap_getCategoryElementUIntegerArray(char **lines, char *name, char *err_buffer, jmp_buf jump_buffer, bool jump)
{
	...
	for (int i = 0; lines[i]; i++)
		if (OsuMap_stringStartsWith(lines[i], buffer)) {
			...
			if (!result.content) {
				sprintf(err_buffer, "Memory allocation error (%luB)", result.length * sizeof(*result.content));
				longjmp(jump_buffer, true);
			}
...
OsuIntegerVectorArray	OsuMap_getIntegerVectorArray(char *str, char *err_buffer, jmp_buf jump_buffer)
{
	...
	if (!array.content) {
		sprintf(err_buffer, "Memory allocation error (%luB)", array.length * sizeof(*array.content));
		free(elems);
		longjmp(jump_buffer, true);
	}
...
OsuMap_hitObjectSliderInfos	OsuMap_getSliderInfos(char **elems, char *err_buffer, jmp_buf jump_buffer)
{
	...
	if (elems[3]) {
		...
		if (!infos.edgeHitsounds) {
			sprintf(err_buffer, "Memory allocation error (%luB)", OsuMap_getStringArraySize(nbr) + 1);
			free(nbr);
			longjmp(jump_buffer, true);
		}
		for (int i = 0; nbr[i]; i++)
		...
		if (elems[4]) {
			...
			if (!infos.edgeAdditions) {
				sprintf(err_buffer, "Memory allocation error (%luB)", (OsuMap_getStringArraySize(nbr) + 1) * sizeof(*infos.edgeAdditions));
				free(nbr);
				longjmp(jump_buffer, true);
			}
			for (int i = 0; nbr[i]; i++) {
...
OsuMap_hitObject	OsuMap_parseLineToHitObject(char *line, char *err_buffer, jmp_buf jump_buffer)
{
	...
	if (obj.type & HITOBJ_SLIDER) {
		...
		if (!obj.additionalInfos) {
			sprintf(err_buffer, "Memory allocation error (%luB)", sizeof(OsuMap_hitObjectSliderInfos));
			free(elems);
			free(old);
			longjmp(jump_buffer, true);
		}
		*(OsuMap_hitObjectSliderInfos *)obj.additionalInfos = OsuMap_getSliderInfos(&elems[5], err_buffer, jump_buffer);
		...
		if (!obj.additionalInfos) {
			sprintf(err_buffer, "Memory allocation error (%luB)", sizeof(unsigned long));
			free(elems);
			free(old);
			longjmp(jump_buffer, true);
		}
...
OsuMap_hitObjectArray	OsuMap_getCategoryHitObject(OsuMapCategory *category, char *err_buffer, jmp_buf jump_buffer)
{
	...
	if (!elements.content) {
		sprintf(err_buffer, "Memory allocation error (%luB)", elements.length * sizeof(*elements.content));
		longjmp(jump_buffer, true);
	}
